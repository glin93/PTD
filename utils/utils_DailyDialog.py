...
def genClassWordmap(data_name, min_word_freq=2):
    ...
    assert data_name in {'DailyAgent', 'DailyUser'}
    data_path = 'data/newDailyDialog/' + data_name
...
def write_txt(split, obj, src_data, trg_data, data_name):
    ...
    assert data_name in {'DailyAgent', 'DailyUser'}
    with open('data/newDailyDialog/' + data_name + '/src-' + split + '-' + obj + '.txt', 'w') as f:
        for src in src_data:
            f.write(src + '\n')
    with open('data/newDailyDialog/' + data_name + '/tgt-' + split + '-' + obj + '.txt', 'w') as f:
        for trg in trg_data:
            f.write(trg + '\n')
...
def process_NMT(data_name):
    ...
    assert data_name in {'DailyAgent', 'DailyUser'}
    data_path = 'data/newDailyDialog/' + data_name + '/' + data_name
    if data_name[-1] == 't':
...
def get_combine_data(split, wordmap, data_name, nmt_name, preserve_punctuation=True):
    ...
    if preserve_punctuation:
        data_path = 'data/newDailyDialog/' + data_name + '/combine_punc_' + nmt_name + '_' + split + '.json'
    else:
        data_path = 'data/newDailyDialog/' + data_name + '/combine_' + nmt_name + '_' + split + '.json'
    ...
    with open('data/newDailyDialog/' + data_name + '/src-' + split + '-agent.txt', 'r') as f:
        for line in f.readlines():
            agent_his.append(line.strip())
    with open('data/newDailyDialog/' + data_name + '/src-' + split + '-user.txt', 'r') as f:
        for line in f.readlines():
            user_his.append(line.strip())
    with open('data/newDailyDialog/' + data_name + '/pred-' + split + '-agent-for-agent-' + nmt_name + '.txt', 'r') as f:
        # 用agent的生成器去处理agent数据集的结果
        for line in f.readlines():
            agent_for_agent.append(line.strip())
    with open('data/newDailyDialog/' + data_name + '/pred-' + split + '-agent-for-user-' + nmt_name + '.txt', 'r') as f:
        # 用agent的生成器去处理user数据集的结果
        for line in f.readlines():
            agent_for_user.append(line.strip())
    with open('data/newDailyDialog/' + data_name + '/pred-' + split + '-user-for-agent-' + nmt_name + '.txt', 'r') as f:
        # 用user的生成器去处理agent数据集的结果
        for line in f.readlines():
            user_for_agent.append(line.strip())
    with open('data/newDailyDialog/' + data_name + '/pred-' + split + '-user-for-user-' +  nmt_name + '.txt', 'r') as f:
    ...
    random.shuffle(data)
    if preserve_punctuation:
        with open('data/newDailyDialog/' + data_name + '/combine_punc_' + nmt_name + '_' + split + '.json', 'w') as j:
            json.dump(data, j)
    else:
        with open('data/newDailyDialog/' + data_name + '/combine_' + nmt_name + '_' + split + '.json', 'w') as j:
            json.dump(data, j)
...
class CombineDataset(torch.utils.data.Dataset):
    def __init__(self, split, data_name, nmt_name, preserve_punctuation):
        if preserve_punctuation:
            with open('data/newDailyDialog/' + data_name + '/combine_punc_' + nmt_name + '_' + split + '.json', 'r') as j:
                self.data = json.load(j)
                self.len_data = len(self.data)
        else:
            with open('data/newDailyDialog/' + data_name + '/combine_' + nmt_name + '_' + split + '.json', 'r') as j:
                self.data = json.load(j)
                self.len_data = len(self.data)            
...
def write_tsv_for_bert(split, data_name, task, preserve_punctuation, nmt_name=None):
    ...
    if task == 'his':
        if preserve_punctuation:
            data_path = 'data/newDailyDialog/' + data_name + '/his_punc_' + split + '.tsv'
        else:
            data_path = 'data/newDailyDialog/' + data_name + '/his_' + split + '.tsv'
        ...
        if preserve_punctuation:
            df.to_csv('data/newDailyDialog/' + data_name + '/his_punc_' + split + '.tsv', index=False, sep='\t')
        else:
            df.to_csv('data/newDailyDialog/' + data_name + '/his_' + split + '.tsv', index=False, sep='\t')
        ...
        if args['addition_name'] == 'directClassify':
            if preserve_punctuation:
                data_path = 'data/newDailyDialog/' + data_name + '/direct_punc_' + split + '.tsv'
            else:
                data_path = 'data/newDailyDialog/' + data_name + '/direct_' + split + '.tsv'
            if os.path.exists(data_path):
                return False
            ...
            df = pd.DataFrame(data)
            if preserve_punctuation:
                df.to_csv('data/newDailyDialog/' + data_name + '/direct_punc_' + split + '.tsv', index=False, sep='\t')
            else:
                df.to_csv('data/newDailyDialog/' + data_name + '/direct_' + split + '.tsv', index=False, sep='\t')
            logging.info('BERT {} DATA LEN: {} | punc={} '.format(split, len(df), preserve_punctuation))
            ...
            if preserve_punctuation:
                data_path = 'data/newDailyDialog/' + data_name + '/combine_punc_' + split + '.tsv'
            else:
                data_path = 'data/newDailyDialog/' + data_name + '/combine_' + split + '.tsv'
            ...
            df['id'] = range(len(df))
            if preserve_punctuation:
                df.to_csv('data/newDailyDialog/' + data_name + '/combine_punc_' + split + '.csv', index=False)
            else:
                df.to_csv('data/newDailyDialog/' + data_name + '/combine_' + split + '.csv', index=False)
            logging.info('BERT {} DATA LEN: {} | punc={} '.format(split, len(df), preserve_punctuation))
...
if __name__ == "__main__":
    wordmap_path = os.path.join('data/newDailyDialog', 'DailyUser', 'class_wordmap.json')
    embed_path = os.path.join('data/newDailyDialog', 'DailyUser', 'pretrain_embed.pt')

    if os.path.exists(wordmap_path) == False:
        genClassWordmap('DailyUser', min_word_freq=6)

    lang = Lang(wordmap_path)
    if os.path.exists(embed_path) == False:
        process_embedding('DailyUser', lang.word2index)

    for split in ['train', 'val', 'test']:
        for obj in ['user', 'agent']:
            data_path = 'data/newDailyDialog/' + 'DailyUser'
            ...
            if os.path.exists(src_path) == False or os.path.exists(tgt_path) == False:
                process_NMT('DailyUser')

    if args['combine']:
        get_combine_data('train', lang, 'DailyUser', args['nmt_name'], False)
        get_combine_data('val', lang, 'DailyUser', args['nmt_name'], False)
        get_combine_data('test', lang, 'DailyUser', args['nmt_name'], False)

        get_combine_data('train', lang, 'DailyUser', args['nmt_name'], True)
        get_combine_data('val', lang, 'DailyUser', args['nmt_name'], True)
        get_combine_data('test', lang, 'DailyUser', args['nmt_name'], True)
    
    if args['process_bert']:
        write_tsv_for_bert('train', 'DailyUser', 'his', False)
        write_tsv_for_bert('val', 'DailyUser', 'his', False)
        write_tsv_for_bert('test', 'DailyUser', 'his', False)

        write_tsv_for_bert('train', 'DailyUser', 'his', True)
        write_tsv_for_bert('val', 'DailyUser', 'his', True)
        write_tsv_for_bert('test', 'DailyUser', 'his', True)

        write_tsv_for_bert('train', 'DailyUser', 'combine', False, args['nmt_name'])
        write_tsv_for_bert('val', 'DailyUser', 'combine', False, args['nmt_name'])
        write_tsv_for_bert('test', 'DailyUser', 'combine', False, args['nmt_name'])

        write_tsv_for_bert('train', 'DailyUser', 'combine', True, args['nmt_name'])
        write_tsv_for_bert('val', 'DailyUser', 'combine', True, args['nmt_name'])
        write_tsv_for_bert('test', 'DailyUser', 'combine', True, args['nmt_name'])
