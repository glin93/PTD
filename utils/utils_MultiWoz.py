...
with open('data/newAgentUser/mapping.pair') as fin:
...
def normalize(text):
    ...
    # modified
    text = re.sub(r"b &b", "bed and breakfast", text)
    text = re.sub(r"a & b", "a&b", text)
    text = re.sub(r"b & b", "bed and breakfast", text)
    text = re.sub(r" & ", " and ", text)
    text = re.sub(r" # ", " ", text)
    tokens = text.split()
    for i in range(len(tokens)):
        if tokens[i] != '[value_count]' and tokens[i].find('[value_count]') != -1:
            tokens[i] = '[value_count]'
    text = ' '.join(tokens)
...
def genClassWordmap(data_name, min_word_freq=2):
    ...
    assert data_name in {'newAgent', 'newUser', 'newSpAgent', 'newSpUser'}
    data_path = 'data/newAgentUser/' + data_name
        ...
        if data_name[-1] == 't':
            for item in data:
                for turn in item['logs']:         
                    word_freq.update(normalize(turn['user']).split())
                    for subturn in turn['agent']:
                        word_freq.update(normalize(subturn['text']).split())
        else:
            for item in data:
                for turn in item['logs']:
                    word_freq.update(normalize(turn['agent']).split())
...
def write_txt(split, obj, src_data, trg_data, data_name):
    ...
    assert data_name in {'newAgent', 'newUser', 'newSpAgent', 'newSpUser'}
    with open('data/newAgentUser/' + data_name + '/src-' + split + '-' + obj + '.txt', 'w') as f:
        for src in src_data:
            f.write(src + '\n')
    with open('data/newAgentUser/' + data_name + '/tgt-' + split + '-' + obj + '.txt', 'w') as f:
        for trg in trg_data:
            f.write(trg + '\n')
...
def process_NMT(data_name):
    ...
    assert data_name in {'newAgent', 'newUser', 'newSpAgent', 'newSpUser'}
    data_path = 'data/newAgentUser/' + data_name + '/' + data_name
    if data_name[-1] == 't':
...
def get_combine_data(split, wordmap, data_name, nmt_name, preserve_punctuation=True):
    ...
    if preserve_punctuation:
        data_path = 'data/newAgentUser/' + data_name + '/combine_punc_' + nmt_name + '_' + split + '.json'
    else:
        data_path = 'data/newAgentUser/' + data_name + '/combine_' + nmt_name + '_' + split + '.json'
    ...
    with open('data/newAgentUser/' + data_name + '/src-' + split + '-agent.txt', 'r') as f:
        for line in f.readlines():
            agent_his.append(line.strip())
    with open('data/newAgentUser/' + data_name + '/src-' + split + '-user.txt', 'r') as f:
        for line in f.readlines():
            user_his.append(line.strip())
    with open('data/newAgentUser/' + data_name + '/pred-' + split + '-agent-for-agent-' + nmt_name + '.txt', 'r') as f:
        # 用agent的生成器去处理agent数据集的结果
        for line in f.readlines():
            agent_for_agent.append(line.strip())
    with open('data/newAgentUser/' + data_name + '/pred-' + split + '-agent-for-user-' + nmt_name + '.txt', 'r') as f:
        # 用agent的生成器去处理user数据集的结果
        for line in f.readlines():
            agent_for_user.append(line.strip())
    with open('data/newAgentUser/' + data_name + '/pred-' + split + '-user-for-agent-' + nmt_name + '.txt', 'r') as f:
        # 用user的生成器去处理agent数据集的结果
        for line in f.readlines():
            user_for_agent.append(line.strip())
    with open('data/newAgentUser/' + data_name + '/pred-' + split + '-user-for-user-' +  nmt_name + '.txt', 'r') as f:
    ...
    random.shuffle(data)
    if preserve_punctuation:
        with open('data/newAgentUser/' + data_name + '/combine_punc_' + nmt_name + '_' + split + '.json', 'w') as j:
            json.dump(data, j)
    else:
        with open('data/newAgentUser/' + data_name + '/combine_' + nmt_name + '_' + split + '.json', 'w') as j:
            json.dump(data, j)
...
class CombineDataset(torch.utils.data.Dataset):
    def __init__(self, split, data_name, nmt_name, preserve_punctuation):
        if preserve_punctuation:
            with open('data/newAgentUser/' + data_name + '/combine_punc_' + nmt_name + '_' + split + '.json', 'r') as j:
                self.data = json.load(j)
                self.len_data = len(self.data)
        else:
            with open('data/newAgentUser/' + data_name + '/combine_' + nmt_name + '_' + split + '.json', 'r') as j:
                self.data = json.load(j)
                self.len_data = len(self.data)            
...
def write_tsv_for_bert(split, data_name, task, preserve_punctuation, nmt_name=None):
    ...
    if task == 'his':
        if preserve_punctuation:
            data_path = 'data/newAgentUser/' + data_name + '/his_punc_' + split + '.tsv'
        else:
            data_path = 'data/newAgentUser/' + data_name + '/his_' + split + '.tsv'
        ...
        if preserve_punctuation:
            df.to_csv('data/newAgentUser/' + data_name + '/his_punc_' + split + '.tsv', index=False, sep='\t')
        else:
            df.to_csv('data/newAgentUser/' + data_name + '/his_' + split + '.tsv', index=False, sep='\t')
        ...
        if args['addition_name'] == 'directClassify':
            if preserve_punctuation:
                data_path = 'data/newAgentUser/' + data_name + '/direct_punc_' + split + '.tsv'
            else:
                data_path = 'data/newAgentUser/' + data_name + '/direct_' + split + '.tsv'
            if os.path.exists(data_path):
                return False
            ...
            df = pd.DataFrame(data)
            if preserve_punctuation:
                df.to_csv('data/newAgentUser/' + data_name + '/direct_punc_' + split + '.tsv', index=False, sep='\t')
            else:
                df.to_csv('data/newAgentUser/' + data_name + '/direct_' + split + '.tsv', index=False, sep='\t')
            logging.info('BERT {} DATA LEN: {} | punc={} '.format(split, len(df), preserve_punctuation))
            ...
            if preserve_punctuation:
                data_path = 'data/newAgentUser/' + data_name + '/combine_punc_' + split + '.tsv'
            else:
                data_path = 'data/newAgentUser/' + data_name + '/combine_' + split + '.tsv'
            ...
            df['id'] = range(len(df))
            if preserve_punctuation:
                df.to_csv('data/newAgentUser/' + data_name + '/combine_punc_' + split + '.csv', index=False)
            else:
                df.to_csv('data/newAgentUser/' + data_name + '/combine_' + split + '.csv', index=False)
            logging.info('BERT {} DATA LEN: {} | punc={} '.format(split, len(df), preserve_punctuation))
...
if __name__ == "__main__":
    wordmap_path = os.path.join('data/newAgentUser', 'newSpUser', 'class_wordmap.json')
    embed_path = os.path.join('data/newAgentUser', 'newSpUser', 'pretrain_embed.pt')

    if os.path.exists(wordmap_path) == False:
        genClassWordmap('newSpUser')

    lang = Lang(wordmap_path)
    if os.path.exists(embed_path) == False:
        process_embedding('newSpUser', lang.word2index)

    for split in ['train', 'val', 'test']:
        for obj in ['user', 'agent']:
            data_path = 'data/newAgentUser/' + 'newSpUser'
            ...
            if os.path.exists(src_path) == False or os.path.exists(tgt_path) == False:
                process_NMT('newSpUser')
    
    if args['combine']:
        get_combine_data('train', lang, 'newSpUser', args['nmt_name'], False)
        get_combine_data('val', lang, 'newSpUser', args['nmt_name'], False)
        get_combine_data('test', lang, 'newSpUser', args['nmt_name'], False)

        get_combine_data('train', lang, 'newSpUser', args['nmt_name'], True)
        get_combine_data('val', lang, 'newSpUser', args['nmt_name'], True)
        get_combine_data('test', lang, 'newSpUser', args['nmt_name'], True)

    if args['process_bert']:
        write_tsv_for_bert('train', 'newSpUser', 'his', False)
        write_tsv_for_bert('val', 'newSpUser', 'his', False)
        write_tsv_for_bert('test', 'newSpUser', 'his', False)

        write_tsv_for_bert('train', 'newSpUser', 'his', True)
        write_tsv_for_bert('val', 'newSpUser', 'his', True)
        write_tsv_for_bert('test', 'newSpUser', 'his', True)

        write_tsv_for_bert('train', 'newSpUser', 'combine', False, args['nmt_name'])
        write_tsv_for_bert('val', 'newSpUser', 'combine', False, args['nmt_name'])
        write_tsv_for_bert('test', 'newSpUser', 'combine', False, args['nmt_name'])

        write_tsv_for_bert('train', 'newSpUser', 'combine', True, args['nmt_name'])
        write_tsv_for_bert('val', 'newSpUser', 'combine', True, args['nmt_name'])
        write_tsv_for_bert('test', 'newSpUser', 'combine', True, args['nmt_name'])
